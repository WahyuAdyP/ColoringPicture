...
extension PaintingImageView {
    ...
    func buckerFill(_ touchPoint: CGPoint, replacementColor: UIColor, isTap: Bool = true, checkSame: ((Bool) -> Void)? = nil){
        buckerFill(touchPoint, replacementGradientColor: [replacementColor, replacementColor], isTap: isTap, checkSame: checkSame)
    }
    
    func buckerFill(_ touchPoint:CGPoint, replacementGradientColor: [UIColor], isTap: Bool, checkSame: ((Bool) -> Void)? = nil) {
...
//        let img = OpenCVWrapper.floodFill(self.image, point: touchPointInImage, replacementColor: replacementColor)
        let colors = NSMutableArray(array: replacementGradientColor.map( { $0.cgColor }))
//        let whiteImg = self.image?.floodFill(from: touchPointInImage, with: UIColor.white, andTolerance: 0, useAntiAlias: false)
//        let gradImg = image?.floodFill(from: touchPointInImage, withGradientColor: colors, andTolerance: 100, useAntiAlias: false)
//        let img = image?.testes((image?.size)!, with: touchPointInImage, withGradient: colors, withTolerance: 0)
        let img = image?.floodFill(from: touchPointInImage, withGradientColor: colors, andTolerance: 0, andActionTap: isTap, useAntiAlias: false, sameImage: { (isSame) in
            checkSame?(isSame)
        })
        self.image = img
