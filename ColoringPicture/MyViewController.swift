...
struct Piece {
    var touchUndo: CGPoint = CGPoint.zero
    var touchRedo: CGPoint = CGPoint.zero
    var colorUndo: [UIColor] = [UIColor]()
    var colorRedo: [UIColor] = [UIColor]()
}
...
//    var isUndo: Bool = false
    ...
    var historyImg: [HistoryImage] = [HistoryImage]()
    var panHisImg: HistoryImage?
    
    var colors = [UIColor]()
    var gradientColors = [[UIColor]]()
    var selectedColor: UIColor = UIColor.red
    var selectedGradientColor: [UIColor] = [UIColor.white, UIColor.red]
    
    var historyPieces = [Piece]()
...
//    var imageUrl: URL?
    ...
    override func viewDidLoad() {
        ...
        colors = [
            ...
            UIColor.purple,
            UIColor.magenta,
            UIColor.init(red: 17/255, green: 17/255, blue: 255/255, alpha: 1.0),
            UIColor.cyan,
            UIColor.green,
            UIColor.brown,
        ...
        gradientColors = [
            ...
            [UIColor.white, UIColor.init(red: 17/255, green: 17/255, blue: 255/255, alpha: 1.0)],
            [UIColor.white, UIColor.cyan],
            [UIColor.white, UIColor.green],
            [UIColor.white, UIColor.brown],
            [UIColor.white, UIColor.init(red: 17/255, green: 17/255, blue: 17/255, alpha: 1.0)],
    ...
    @objc func tapToFill(_ sender: UITapGestureRecognizer) {
        ...
        if color.isEqual(to: selectedColor, strict: false) && !isGradient {
            return
        }
        if color.isEqual(to: UIColor.black, strict: false) {
            return
        }
        
//        var layerPiece = LayerPiece()
//        layerPiece.undoColor = color
//        layerPiece.redoColor = selectedColor
//        
//        
...
//        isUndo = false
        ...
        var hisImg = HistoryImage()
        hisImg.undoImg = paintView.image!.pngData()
        
        if isGradient {
            paintView.buckerFill(point, replacementGradientColor: selectedGradientColor, isTap: true)
        } else {
            paintView.buckerFill(point, replacementColor: selectedColor, isTap: true)
        }
        hisImg.redoImg = paintView.image!.pngData()
        
        if currentPosition == historyImg.count {
            historyImg.append(hisImg)
        } else {
            var indexRemove = currentPosition
            while indexRemove < historyImg.count {
                historyImg.remove(at: indexRemove)
                indexRemove += 1
            }
            historyImg.append(hisImg)
        }
        currentPosition = historyImg.count
    }
...
//            panHistoryLayers = [LayerPiece]()
            panHisImg = HistoryImage()
            panHisImg?.undoImg = paintView.image!.pngData()
        }
        ...
        if !color.isEqual(to: UIColor.black, strict: false) {
            if isGradient {
                paintView.buckerFill(point, replacementGradientColor: selectedGradientColor, isTap: false)
            } else {
                if !color.isEqual(to: selectedColor, strict: false) {
                    paintView.buckerFill(point, replacementColor: selectedColor, isTap: false)
                }
            }
        }
...
//            
//            if isGradient {
//                paintView.buckerFill(point, replacementGradientColor: selectedGradientColor)
//            } else {
//                paintView.buckerFill(point, replacementColor: selectedColor)
//            }
//        }
        ...
        if sender.state == .ended {
            panHisImg?.redoImg = paintView.image!.pngData()
            if currentPosition == historyImg.count {
                historyImg.append(panHisImg!)
            } else {
                var indexRemove = currentPosition
                while indexRemove < historyImg.count {
                    historyImg.remove(at: indexRemove)
...
//        }
    ...
    func animateMultiColor(currentIndex: Int, endIndex: Int, data: [LayerPiece]) {
        ...
        self.paintView.buckerFill(aHistory.point, replacementColor: color!, isTap: true)
    ...
    @objc func undo(_ button: UIButton) {
        if currentPosition > 0 {
            currentPosition -= 1
            paintView.image = UIImage(data: historyImg[currentPosition].undoImg!)
...
//            }
    ...
    @objc func redo(_ button: UIButton) {
        if currentPosition < historyImg.count {
            paintView.image = UIImage(data: historyImg[currentPosition].redoImg!)
//            if let aHistory = historyLayer[currentPosition] as? LayerPiece {
...
//        setZoomScale()
    ...
    func setImageOpenCV() {
        ...
        let imageCV = OpenCVWrapper.scanImage(newImage)
        let argbImg = createARGBBitmap(image: imageCV!)
        
        let argbData = argbImg!.pngData()!
        _ = [UInt8](argbData)
        let openCVData = imageCV!.pngData()!
        _ = [UInt8](openCVData)
...
extension MyViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.item < colors.count {
            ...
            self.selectedGradientColor = gradientColors[indexPath.item - colors.count]
